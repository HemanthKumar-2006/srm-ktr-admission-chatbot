...
class DeviceConfig:
    use_gpu: bool = True
    gpu_device_id: int = 0              # CUDA device index (0 = first GPU)
    gpu_memory_fraction: float = 0.9    # Fraction of VRAM to allow

    def __post_init__(self) -> None:
        if self.gpu_device_id < 0:
            raise ValueError(f"gpu_device_id must be >= 0, got {self.gpu_device_id}")
        if not (0.0 < self.gpu_memory_fraction <= 1.0):
            raise ValueError(
                f"gpu_memory_fraction must be in (0.0, 1.0], got {self.gpu_memory_fraction}"
            )

    @classmethod
    def from_env(cls) -> "DeviceConfig":
        return cls(
            use_gpu=_bool_env("USE_GPU", cls.use_gpu),
            gpu_device_id=_int_env("GPU_DEVICE_ID", cls.gpu_device_id),
            gpu_memory_fraction=_float_env("GPU_MEMORY_FRACTION", cls.gpu_memory_fraction),
        )

    @property
    def torch_device(self) -> str:
        """Ready-to-use string for torch.device() / HuggingFace device= arg."""
        return f"cuda:{self.gpu_device_id}" if self.use_gpu else "cpu"

    @property
    def use_fp16(self) -> bool:
        """FP16 only makes sense on GPU — halves VRAM with negligible accuracy loss."""
        return self.use_gpu
...
class Settings:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "rag.device.gpu_memory_fraction":   self.rag.device.gpu_memory_fraction,
